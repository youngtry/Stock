...
@interface WechatViewController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
@property (weak, nonatomic) IBOutlet UITextField *wechatNickname;
@property (weak, nonatomic) IBOutlet UITextField *verifyInput;

@property (weak, nonatomic) IBOutlet UITextField *wechatID;

@property (weak, nonatomic) IBOutlet UIButton *sendVerfiyBtn;
...
- (void)viewDidLoad {
    ...
    self.title = Localize(@"Set_Wechat_Pay");
    self.update1 = nil;
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    self.update1 = nil;
    [_sendVerfiyBtn setTitle:Localize(@"Send_Verify") forState:UIControlStateNormal];
...
- (IBAction)clickSaveBtn:(id)sender {
    ...
    vc.block = ^(NSString* token) {
        if(token.length>0){
            ...
            NSDictionary* params = @{@"name":weakSelf.wechatNickname.text,
                                     @"wechat_id":weakSelf.wechatID.text,
...
-(BOOL)isInputAll{
    if(self.wechatNickname.text.length == 0){
        return NO;
    }
    if(self.wechatID.text.length == 0){
        return NO;
    }
