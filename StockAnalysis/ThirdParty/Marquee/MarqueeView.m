...
//        [self addSubview:tagLabel];
        ...
        self.index = 1;
        self.firstBtn = [self btnframe:CGRectMake(10, 0, self.width, self.height)  titleColor:_titleColor action:@selector(clickBtn:)];
        ...
        [NSTimer scheduledTimerWithTimeInterval:1.5 target:self selector:@selector(nextAd) userInfo:nil repeats:YES];
        
//        [self startAnimationIfNeeded:self.firstBtn.titleLabel];
...
-(void)startAnimationIfNeeded:(UILabel*) titleLabel{
    //取消、停止所有的动画
...
-(void)nextAd{
    ...
    self.moreBtn = [self btnframe: CGRectMake(10, self.bounds.size.height,self.width, self.height)  titleColor:_titleColor action:@selector(clickBtn:)];
...
//        [self startAnimationIfNeeded:self.moreBtn.titleLabel];
